$(function () {
    ...
    $('.mobile_menu').click(function () {
        if ($('.dashboard_nav, .dashboard_nav_normalize').css('left') !== '-240px') {
            $('.dashboard_nav, .dashboard_nav_normalize').animate({left: '-240px'}, 300);
            $('.dashboard_fix').animate({'margin-left': '0px'}, 300);
        } else {
            $('.dashboard_nav, .dashboard_nav_normalize').animate({left: '0px'}, 300);
            $('.dashboard_fix').animate({'margin-left': '240px'}, 300);
        }
    });
    ...
    $(window).load(function () {
        HeaderRender('normalize_height_course');
        HeaderRender('normalize_height_time');
    });
...
function studentClassFree(data) {
    if (data.classfree[0] === 'free') {
        ...
        var freeBtn = "<span class='icon-checkbox-checked'>AULA CONCLUÃDA</span>";
