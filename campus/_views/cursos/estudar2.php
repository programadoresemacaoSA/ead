<div class="wc_ead">
    ...
    if (!$Read->getResult()):
        $_SESSION['wc_ead_alert'] = ['red', "Ooops, você ainda não tem esse curso!", "Então, {$_SESSION['userLogin']['user_name']}, para estudar o <strong>Curso {$course_title}</strong>, antes você precisa adquiri-lo. Clique no botão <strong><a target='_blank' title='Saiba Mais' alt='Saiba Mais' href='". BASE . "/{$CourseFree}/{$course_name}'>SAIBA MAIS</a></strong> para conhecer uma oferta especial que preparamos para você!"];
        $visitor = true;
    else:
        extract($Read->getResult()[0]);
...
//        $ClassStudenCount = $Read->getResult()[0]['ClassStudentCount'];
        ...
        $Read->FullRead("SELECT COUNT(student_class_id) as ClassStudentCountView FROM " . DB_EAD_STUDENT_CLASSES . " WHERE user_id = :user AND course_id = :course","user={$user_id}&course={$course_id}");
        ...
        $Read->FullRead("SELECT COUNT(student_quiz_id) as QuizStudentCountView FROM " . DB_EAD_STUDENT_QUIZ . " WHERE user_id = :user AND course_id = :course","user={$user_id}&course={$course_id}");
        $QuizStudenCountView = $Read->getResult()[0]['QuizStudentCountView'];

        $Read->FullRead("SELECT COUNT(student_quiz_id) as QuizStudentCount FROM " . DB_EAD_STUDENT_QUIZ . " WHERE user_id = :user AND course_id = :course AND student_quiz_check IS NOT NULL","user={$user_id}&course={$course_id}");
        $QuizStudenCount = $Read->getResult()[0]['QuizStudentCount'];
        ...
        <div class="dash_view_course_header gradient_blue" style="background: <?= $course_color; ?> !important;">
            ...
            <?php else: ?>
                <div class='dash_view_course_header_cta'>
                    ...
                    <a href="<?= BASE; ?>/<?= $CourseFree; ?>/<?= $course_name; ?>" target="_blank" alt="Saiba Mais" title="Saiba Mais" class='btn_cta btn_blue radius icon-stack'>SAIBA MAIS</a>
                </div>
            <?php endif; ?>
        </div>
</div>
                        ...
                        <div class="dash_content">
                            ...
                            if (!$Read->getResult()):
                                ...
                                foreach ($Read->getResult() as $CLASS):
                                    ...
                                    ?>
                                    <article class="dash_view_course_module_class <?= (!empty($ClassPending) && $ClassPending['class_id'] == $class_id ? 'pending' : ''); ?>">
                                        <a class="dash_view_course_module_class_link" title="Acessar Aula <?= $class_title; ?>" href="campus.php?wc=cursos/tarefa&id=<?= $CourseId; ?>&class=<?= $class_id; ?>">
                                            <h3 class="dash_view_course_module_class_row title <?= $ClassCheck ? "icon-checkbox-checked" : 'icon-checkbox-unchecked'; ?>"><?= $class_title; ?></h3>
                                            <p class="dash_view_course_module_class_row icon-hour-glass"><?= $class_time; ?> min.</p>
                                            <p class="dash_view_course_module_class_row icon-eye"><?= $ClassPlay; ?></p>
                                            <p class="dash_view_course_module_class_row"><span class="icon-bubbles4"><?= $TaskSupport; ?></span></p>
                                        </a>
                                    </article>
                                <?php
                                endforeach;
                            ...
                            ?>
                            <?php
                            $Read->ExeRead(DB_EAD_QUIZ, "WHERE module_id = :module ORDER BY quiz_order ASC","module={$module_id}");
                            if (!$Read->getResult()):
                                echo "<div class='trigger trigger_info trigger_none al_center icon-info'>Ainda não existem quiz cadastradas para o módulo {$module_title}!</div>";
                            else:
                                foreach ($Read->getResult() as $Quiz):
                                    $Read->FullRead("SELECT quiz_item_id FROM " . DB_EAD_QUIZ_ITEMS . " WHERE module_id = :module", "module={$module_id}");
                                    if ($Read->getResult()):
                                        $quiz_item_id = $Read->getResult()[0]['quiz_item_id'];
                                    endif;
                                    extract($Quiz); ?>
                                    <article class="dash_view_course_module_class <?= (!empty($ClassPending) && $ClassPending['quiz_id'] == $quiz_id ? 'pending' : ''); ?>">
                                        <a class="dash_view_course_module_class_link" title="Acessar Aula <?= $quiz_title; ?>" href="campus.php?wc=cursos/quiz&id=<?= $CourseId; ?>&quiz=<?= $quiz_id; ?>">
                                            <h3 class="dash_view_course_module_class_row title <?= $ClassCheck ? "icon-question" : "icon-question"; ?>"><?= $quiz_title; ?></h3>
                                            <p class="dash_view_course_module_class_row">-</p>
                                            <p class="dash_view_course_module_class_row icon-eye"><?= $ClassPlay; ?></p>
                                            <p class="dash_view_course_module_class_row"><span>-</span></p>
                                        </a>
                                    </article>
                                <?php
                                endforeach;
