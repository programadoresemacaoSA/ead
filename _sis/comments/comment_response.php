        ...
        <div class="panel" style="padding: 30px;">
            ...
            if (!$Read->getResult()):
                ...
                elseif ($Comm['class_id']):
                    $Read->FullRead("SELECT class_name, class_title FROM " . DB_CLASS . " WHERE class_id = :id", "id={$Comm['class_id']}");
                    $Link = BASE . "/campus/campus.php?wc=cursos/play&id={$Read->getResult()[0]['class_name']}";
                    $Title = $Read->getResult()[0]['class_title'];
                elseif ($Comm['course_id']):
                    $Read->FullRead("SELECT course_name, course_title FROM " . DB_EAD_COURSES . " WHERE course_id = :id", "id={$Comm['course_id']}");
                    $Link = BASE . "/curso/{$Read->getResult()[0]['course_name']}";
                    $Title = $Read->getResult()[0]['course_title'];
                endif;
