<aside class="product-single-aside">
    ...
    <section class="aside-section mead_side">
        ...
        <?php
        $Read->ExeRead(DB_CATEGORIES, "WHERE category_parent IS NULL AND category_id IN(SELECT post_category FROM " . DB_POSTS . " WHERE post_status = 1 AND post_private = 1 AND post_date <= NOW()) ORDER BY category_title ASC");
        if (!$Read->getResult()):
            echo Erro("Ainda não existem sessões cadastradas!", E_USER_NOTICE);
        else:
            echo "<ul>";
            foreach ($Read->getResult() as $Ses):
                echo "<li><a title='artigos-privados/{$Ses['category_name']}' href='" . BASE . "/artigos-privados/{$Ses['category_name']}'>&raquo; {$Ses['category_title']}</a></li>";
                ...
                if ($Read->getResult()):
                    foreach ($Read->getResult() as $Cat):
                        echo "<li><a title='artigos-privados/{$Cat['category_name']}' href='" . BASE . "/artigos-privados/{$Cat['category_name']}'>&raquo;&raquo; {$Cat['category_title']}</a></li>";
                    endforeach;
                endif;
            endforeach;
            echo "</ul>";
    ...
    <section class="aside-section mead_side">
        ...
        <ul class="articles-list">
            <?php
            $Read->ExeRead(DB_POSTS, "WHERE post_status = 1 AND post_private = 1 AND post_date <= NOW() ORDER BY post_views DESC, post_date DESC LIMIT 5");
            if (!$Read->getResult()):
