    ...
    switch ($Case):
        case 'cupom_manage':
            ...
            elseif (!Check::Data($PostData['cp_start']) || !Check::Data($PostData['cp_end'])):
                ...
                $Update->ExeUpdate(DB_EAD_COUPONS, $PostData, "WHERE cp_id = :id", "id={$CouponId}");
        ...
        case 'coupon_remove':
            $Delete->ExeDelete(DB_EAD_COUPONS, "WHERE cp_id = :del_id", "del_id={$PostData['del_id']}");
            $jSON['success'] = true;
            break;
        ...
        case 'manager':
            ...
            $PostData['course_feature'] = (!empty($PostData['course_feature']) ? '1' : '0');
        ...
        case 'class_add':
            ...
            $PostData['class_free'] = (!empty($PostData['class_free']) ? '1' : '0');
        ...
        case 'student_course_add':
            ...
            $Read->ExeRead(DB_EAD_COURSES, "WHERE course_id = :id", "id={$PostData['course_id']}");
            if ($Read->getResult()):
                //ORDER ITENS CREATE
                $CartOrdeItens = [
                    'order_id' => $OrderCreateId,
                    'course_id' => $Read->getResult()[0]['course_id'],
                    'item_name' => $Read->getResult()[0]['course_title'],
                    'item_price' => '0.00',
                    'item_amount' => 1
                ];
                $Create->ExeCreate(DB_EAD_ORDERS_ITEMS, $CartOrdeItens);
            endif;
        ...
        case 'course_certification':
            ...
            unset($PostData['course_id'], $PostData['course_certification_cover']);
            ...
            $CertificationCover = (!empty($_FILES['course_certification_cover']) ? $_FILES['course_certification_cover'] : null);
            $Read->FullRead("SELECT course_certification_cover FROM " . DB_EAD_COURSES . " WHERE course_id=:id AND course_certification_cover IS NOT NULL", "id={$CourseId}");
            if ($CertificationCover):
                if ($Read->getResult() && !empty($Read->getResult()[0]['course_certification_cover']) && file_exists("../../uploads/{$Read->getResult()[0]['course_certification_cover']}") && !is_dir("../../uploads/{$Read->getResult()[0]['course_certification_cover']}")):
                    unlink("../../uploads/{$Read->getResult()[0]['course_certification_cover']}");
                endif;

                $Upload = new Upload('../../uploads/');
                $Upload->Image($CertificationCover, md5($CourseId), '500', 'courses_cover');
                $PostData['course_certification_cover'] = $Upload->getResult();
            elseif (!$Read->getResult()):
                $jSON['trigger'] = AjaxErro("<b class='icon-warning'>OPPSSS:</b> Você esqueceu de enviar a imagem cover do cetificado. Falta apenas ela para atualizar!", E_USER_WARNING);
                $jSON['error'] = true;
                break;
            endif;
        ...
        case 'course_certification_remove':
            ...
            $Read->FullRead("SELECT course_certification_cover FROM " . DB_EAD_COURSES . " WHERE course_id=:id", "id={$CourseId}");
            if ($Read->getResult() && !empty($Read->getResult()[0]['course_certification_cover']) && file_exists("../../uploads/{$Read->getResult()[0]['course_certification_cover']}") && !is_dir("../../uploads/{$Read->getResult()[0]['course_certification_cover']}")):
                unlink("../../uploads/{$Read->getResult()[0]['course_certification_cover']}");
            endif;

            $ReserCertificationCover = ['course_certification_workload' => null, 'course_certification_request' => null, 'course_certification_cover' => null];
            $Update->ExeUpdate(DB_EAD_COURSES, $ReserCertificationCover, "WHERE course_id=:id", "id={$CourseId}");
